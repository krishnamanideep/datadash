...
export function LocationTable({ pollingStations }: LocationTableProps) {
  if (pollingStations.length > 0) {
    console.log('First station data:', pollingStations[0]);
  }
  return (
            ...
            {pollingStations.map((station) => {
              ...
              return (
                <tr key={station.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-sm font-semibold text-purple-600">{station.category || 'N/A'}</td>
                  <td className="px-6 py-4 text-sm font-semibold text-green-600">{station.strongestParty || 'N/A'}</td>
